...
def get_training_data_converage(indir, model):
    ...
    for index, image in enumerate(x_train):
        target_class = train_label[index]
        exp = explainer.explain(image, target_class)
        # if not (target_class in class_exp.keys()):
        #     class_exp[target_class] = exp
        # else:
        #     for key in exp.keys():
        #         class_exp[target_class][key] += exp[key]
    # json_str = json.dumps(class_exp)
    # with open('exp_train_data.json', 'w') as json_file:
...
if __name__ == '__main__':
    ...
    get_training_data_converage(args.i, model)
