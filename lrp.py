...
from keras.layers import InputLayer, MaxPooling2D, Conv2D
...
class LRP:
    ...
    def explain(self, image, target_class):
        ...
        real_relevances[output_layer.name] = target_activations
        ...
        # adjust relevance to get true layer relevance
        for t_key in relevances.keys():
            t_value = relevances[t_key]
            if t_key == output_layer.name:
                next_value = t_value
                continue
            real_relevances[t_key] = next_value
            next_value = t_value
        # for o_key in real_relevances.keys():
        #     print('***', o_key, '***', real_relevances[o_key].shape)
        return real_relevances
        # print(new_r.shape)
    ...
    def _get_layer_before(self, layer):
        ...
        # print(type(in_node[0].inbound_layers))
        # print(type(in_node))
        if isinstance(in_node[0].inbound_layers,list):
            if len(in_node) > 0 and len(in_node[0].inbound_layers) > 0:
                previous_layer = in_node[0].inbound_layers[0]
        else:
            if len(in_node) > 0:
                previous_layer = in_node[0].inbound_layers
    ...
    def get_layers_outputs(self, model, input):
        ...
        for layer in layers:
            ...
            # print(layer.name, '***layer out***', layer_output.shape)
        ...
        # print('***input shape***',model.layers[0].input.shape)
        activations['conv2d_1_input'] = np.expand_dims(input, axis=0)#model.layers[0].input
        return activations
