...
class Mutators():
    ...
    @staticmethod
    def search_params(param_range, image, transformation, center_neuron_cov_path, fetch_function, coverage_function):
        def fit_func(param):
            def distance(ori_img, follow_img):
                # print(type(ori_img), type(follow_img), param, param_range[0], param_range[-1])
                coverage_one, metadata_one = fetch_function(
                    follow_img
                )
                mutated_coverage_list = coverage_function(coverage_one)

                return diff_per

            # print(str(transformation), param)
            new_image = transformation(image, param)
            ditance_num = distance(image, new_image)
            return diff_per

        pso = PSO(n_dim=1, pop=50, max_iter=100, lb=[param_range[0], ], ub=[param_range[-1], ], fitness=fit_func)
        best_x = pso.main()
        # print(best_x, best_y)
        return best_x
    ...
    def mutate_one(ref_img, img, cl, l0_ref, linf_ref, try_num=50):
        ...
        for ii in range(try_num):
            ...
            if cl == 0:  # 0: can choose class A and B
                ...
                # param = Mutators.search_params(params, copy.deepcopy(img), center_neuron_cov_path, fetch_function, coverage_function)
    ...
    def image_random_mutate(seed, batch_num):
        ...
        img = test[1]
        cl = seed.clss
        distance = seed.seed_distance
        energy = int((1-distance)*batch_num)
        ref_batches = []
        batches = []
        cl_batches = []
        l0_ref_batches = []
        ...
        for i in range(batch_num):
            ref_out, img_out, cl_out, changed, l0_ref, linf_ref = Mutators.mutate_one(ref_img, img, cl,
                                                                                      seed.l0_ref, seed.linf_ref)
            if changed:
