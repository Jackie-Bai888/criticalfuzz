...
def iterate_function(names, critical_neuron, criteria):
    def func(queue, root_seed, parent, mutated_coverage_list, mutated_data_batches, mutated_metadata_list,
        ...
        for idx in range(len(mutated_coverage_list)):
            ...
            # else:
            critical_ne_ls = dict2list(critical_neuron[int(parent.ground_truth)].values())
...
def out_critical_coverage(queue):
    for class_label in queue.coverage_critical_neuron.keys():
        cove_1_num, cove_2_num = 0, 0
        class_cov = queue.coverage_critical_neuron[class_label]
        for key in class_cov.keys():
            layer_cov = class_cov[key]
            for index, cv in enumerate(layer_cov):
                if cv == 2:
                    cove_2_num += 1
                if cv == 1:
                    cove_1_num += 1
        print('**** has coveraged %f critical neurons of label %s ****' % (cove_2_num/(cove_2_num+cove_1_num), class_label))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', help='input seed directory', default='/common/hahabai/code/deepfuzzer/test_seeds/mnist_seeds/')
    ...
    dry_run(args.i, dry_run_fetch, coverage_function, queue, critical_neuron, args.criteria)
    # out_critical_coverage(queue)
    # For each seed, compute the coverage and check whether it is a "bug", i.e., adversarial example
    image_iterate_function = iterate_function(model_names, critical_neuron, args.criteria)
